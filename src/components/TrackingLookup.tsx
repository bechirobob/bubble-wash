...
import { formatDate } from "@/lib/public-ui";
...
export function TrackingLookup() {
  ...
  async function track(event: FormEvent<HTMLFormElement>) {
    ...
    if (!reference) { setTone("error"); setResult(null); setStatus("Enter a Bubble Wash reference first."); return; }
    setPending(true);
    setTone("info");
    ...
    try {
      ...
      setTone("success");
    } catch (error) { setTone("error"); setResult(null); setStatus(error instanceof Error ? error.message : "Unable to load tracking details."); }
    finally { setPending(false); }
  }

  return (
    <section className="serviceSection pageShell trackingPageSection" aria-labelledby="tracking-heading">
      <div className="trackingLayout"><form className="serviceForm" onSubmit={track}><label id="tracking-heading">Order reference<input name="trackingId" placeholder="BW-…" autoComplete="off" /></label><button className="button primary" type="submit" disabled={pending}><Search aria-hidden="true" />{pending ? "Checking…" : "Check order"}</button><p className={`status ${tone}`} role="status" aria-live="polite">{status}</p></form>{result ? <article className="trackingResult"><strong className="trackingStatus">{result.status}</strong><p>{result.nextStep}</p><dl className="miniFacts"><div><dt>Reference</dt><dd>{result.id}</dd></div><div><dt>Pickup locality</dt><dd>{result.area}</dd></div><div><dt>Pickup window</dt><dd>{result.routeWindow || "Confirmation pending"}</dd></div><div><dt>Last updated</dt><dd>{formatDate(result.updatedAt || result.createdAt)}</dd></div></dl></article> : null}</div>
    </section>
  );
}
