...
export function EarlyAccessForm() {
  ...
  if (result.kind === "success") {
    return <aside className="servicePanel earlyAccessPanel" id="early-access" aria-live="polite"><p className="sectionLabel">You are on the list</p><h2>{result.updated ? "Your details are up to date." : "We saved your early-access request."}</h2><p>{result.message}</p><dl className="recordList"><div><dt>WhatsApp confirmation</dt><dd>{result.confirmation?.whatsapp === "sent" ? "Sent" : result.confirmation?.whatsapp === "failed" ? "Retry pending" : "Not sent — follow-up needed"}</dd></div><div><dt>Email confirmation</dt><dd>{result.confirmation?.email === "not_requested" ? "Not requested" : result.confirmation?.email === "sent" ? "Sent" : result.confirmation?.email === "failed" ? "Retry pending" : "Not sent — follow-up needed"}</dd></div></dl><button className="button secondary" type="button" onClick={() => setResult({ kind: "idle" })}>Update another signup</button></aside>;
  }
