    ...
    <main className="siteShell">
      ...
      <section className="homeHero pageShell" aria-labelledby="home-title">
        ...
        <aside className="servicePanel" aria-labelledby="service-summary-title">
          ...
          <dl className="recordList">
            ...
            <div><dt>Updates</dt><dd>{automatedUpdatesEnabled ? "WhatsApp and email" : "Tracking and operations follow-up"}</dd></div>
      ...
      <section id="booking" className="serviceSection pageShell" aria-labelledby="booking-heading">
        ...
        <form className="bookingForm serviceForm" onSubmit={submitBooking}>
          ...
          <fieldset className="bookingFieldset"><legend>Collection</legend><div className="formGrid two"><label>Collection area<input name="area" value={bookingArea} onChange={(event) => setBookingArea(event.target.value)} autoComplete="address-level2" required /></label><label>Preferred date<input name="pickupDate" type="date" min={minPickupDate} max={maxPickupDate} required /></label></div><div className="formGrid two"><label>Collection window<select name="pickupWindow" defaultValue="Any available window"><option>Any available window</option><option>Morning pickup</option><option>Afternoon pickup</option><option>Evening pickup</option></select></label><label>Updates<select name="alertPreference" defaultValue={automatedUpdatesEnabled ? "Email + WhatsApp alerts" : "Order tracking and phone follow-up"}>{automatedUpdatesEnabled ? <><option>Email + WhatsApp alerts</option><option>WhatsApp only</option><option>Email only</option></> : <option>Order tracking and phone follow-up</option>}<option>Call me</option></select></label></div></fieldset>
          <fieldset className="bookingFieldset"><legend>Billing</legend><div className="formGrid two"><label>Preferred payment<select name="paymentPreference" defaultValue={onlinePaymentsEnabled ? "Card" : "Bank transfer"}>{onlinePaymentsEnabled ? <><option>Card</option><option>MTN MoMo</option><option>Telecel Cash</option></> : null}<option>Bank transfer</option><option>Invoice me</option></select></label><label>Estimated weight<input value={`${kg} kg per pickup`} readOnly aria-label="Estimated weight from pricing" /></label></div>{!onlinePaymentsEnabled || !automatedUpdatesEnabled ? <div className="comingSoonList" aria-label="Services coming soon">{!onlinePaymentsEnabled ? <div><strong>Card and Mobile Money</strong><span>Coming soon</span><small>Online checkout is being prepared and cannot be selected during the pilot.</small></div> : null}{!automatedUpdatesEnabled ? <div><strong>Automatic WhatsApp and email updates</strong><span>Coming soon</span><small>Operations will use your booking details for manual follow-up during the pilot.</small></div> : null}</div> : null}<p className="bookingContextSummary">{plan} · {zones[zone].label} · {selectedAddons.length ? selectedAddons.map((key) => addons[key].label).join(", ") : "No extras selected"}</p></fieldset>
      ...
      <section className="paymentStrip pageShell" aria-labelledby="payment-heading"><p className="sectionLabel">Payments</p><h3 id="payment-heading">Bank transfer and approved invoicing</h3><p>These are the active pilot payment methods. Card and Mobile Money checkout are coming soon and are not yet available.</p></section>
