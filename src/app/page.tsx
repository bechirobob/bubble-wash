  ...
  return (
    <main>
      ...
      <section id="menu-desk" className="section menuDeskSection soft" aria-labelledby="menu-desk-heading">
        ...
        {activeMenuPanel === "coverage" && <div className="menuPanelContent" role="tabpanel" aria-label="Coverage and vendors"><div className="coverageMenuGrid refinedCoverageGrid coverageDecisionGrid"><div className="panel coverageRoutesPanel"><div className="coveragePanelHead"><span className="badge">Coverage</span><h3>Choose a pickup lane</h3><p>Route groups are listed by dispatch certainty, not shown as a fake map. Tap an area above or type yours to attach the lane to booking.</p></div><div className="coverageRouteList laneRouteList">{coverageGroups.map((group, index) => <article className="coverageRouteGroup laneCard" key={group.title}><span className="laneIndex">{String(index + 1).padStart(2, "0")}</span><div><strong>{group.title}</strong><small>{group.fee}</small></div><ul>{group.areas.map((area) => <li key={area}>{area}</li>)}</ul></article>)}</div></div><aside className="coverageSummaryCard routeDecisionCard" aria-label="Selected pickup lane summary"><span className="badge">{routePreview.zoneLabel}</span><h3>{routePreview.pickup.label}</h3><p>{routePreview.zoneNote}</p><div className="routeFacts routePromiseFacts"><span><b>{routePreview.estimatedDriveMinutes} min</b><small>dispatch window</small></span><span><b>{formatMoney(zones[routePreview.zoneKey].fee)}</b><small>route fee</small></span><span><b>{routePreview.hub.label}</b><small>fulfilment hub</small></span></div><div className="routeDecisionNote"><strong>No fake map here.</strong><span>Dispatch confirms the real driver route after pickup time, bag count, and vendor capacity are known.</span></div><div className="mapActions"><button className="button primary" type="button" onClick={() => { setBookingZone(routePreview.zoneKey); setBookingArea(routePreview.pickup.label); document.getElementById("booking")?.scrollIntoView({ behavior: "smooth" }); }}>Use this lane</button><button className="button secondary" type="button" onClick={() => openMenuPanel("pricing")}>Price this route</button></div></aside></div><div className="vendorShowcaseGrid compactVendorGrid vendorDirectoryList">{vendors.map(([name, area, tagOne, tagTwo, metric, tone]) => <article className="vendorShowcaseCard vendorDirectoryCard" key={name}><div className={`vendorPhoto ${tone}`}><span>{metric}</span></div><div className="vendorDirectoryCopy"><h3>{name}</h3><p>{area}</p><div className="tagRow"><span>{tagOne}</span><span>{tagTwo}</span></div></div><button className="button primary full" type="button" onClick={() => chooseVendor(name)}>Request Service</button></article>)}</div>{formStatus["vendor-choice"] && <p className="status success">{formStatus["vendor-choice"]}</p>}</div>}
