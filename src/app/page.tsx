        ...
        {activeMenuPanel === "pricing" && <div className="menuPanelContent" role="tabpanel" aria-label="Pricing and quote"><div className="plansGrid slimPlans">{plans.map((plan) => {
            ...
            return (
              <article className={plan.badge ? "planCard featured" : "planCard"} key={plan.name}>
                ...
                <button className="button primary full" type="button" onClick={() => choosePlan(plan.name as PlanName)}>{plan.name === "Enterprise" ? "Prepare enterprise quote" : "Use this plan"}</button>
              </article>
            );
