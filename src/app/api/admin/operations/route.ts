...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, metrics: operationsDataMetrics(), privacyRequests: listPrivacyRequests() }, { headers: { "Cache-Control": "private, no-store" } });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const updated = updatePrivacyRequestStatus(id, status);
