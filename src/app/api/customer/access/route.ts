...
import { customerContactMatches, customerSessionCookieName, customerSessionCookieOptions, encodeCustomerSession } from "@/lib/customer-session";
...
export async function POST(request: NextRequest) {
  ...
  if (await isRateLimited(clientKey(request.headers, "customer-access"), 8, 60_000)) {
  ...
  try {
    ...
    const seed = await findSubmissionRecordById(orderId);
    ...
    const records = await readSubmissionRecordsForOrder(orderId);
    ...
    response.cookies.set({
      ...
      value: encodeCustomerSession(orderId, contact),
