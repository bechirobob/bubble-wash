...
const resetId = process.env.BUBBLEWASH_PRODUCTION_RESET_ID;
const resetConfirmation = process.env.BUBBLEWASH_ALLOW_PRODUCTION_RESET;
const maximumBackupAgeMs = 30 * 60_000;

function fail(message) {
  throw new Error(`Production reset refused: ${message}`);
}
...
if (!resetId || !/^production-launch-\d{4}-\d{2}-\d{2}$/.test(resetId)) {
  fail("BUBBLEWASH_PRODUCTION_RESET_ID must identify the dated production launch.");
}
if (resetConfirmation !== resetId) fail("the explicit reset confirmation does not match the reset ID.");
...
try {
  quickCheck();
  const existing = database.prepare("SELECT applied_at, before_counts, after_counts, preserved_counts FROM production_resets WHERE reset_id = ?").get(resetId);
  if (existing) {
    console.log(JSON.stringify({
      ...
      after: countsFor(operationalTables),
  ...
  const reset = database.transaction(() => {
    const before = countsFor(operationalTables);
    const preserved = countsFor(preservedTables);
    for (const table of operationalTables) database.prepare(`DELETE FROM ${table}`).run();
    const after = countsFor(operationalTables);
    if (Object.values(after).some((count) => count !== 0)) fail("one or more operational tables are not empty after deletion.");
    const preservedAfter = countsFor(preservedTables);
    if (JSON.stringify(preservedAfter) !== JSON.stringify(preserved)) fail("a preserved security table changed during the reset.");

    const appliedAt = new Date().toISOString();
    database.prepare(`
      INSERT INTO production_resets (
