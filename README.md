...
`Nightly Production Backup` runs daily at 01:15 UTC, retains seven days of primary VPS copies, clears successful transfer staging, and stores the encrypted off-host artifact in GitHub for 35 days. A failed transfer does not refresh `/api/ready`; readiness is finalized only after GitHub confirms off-host storage.
