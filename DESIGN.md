...
- Assignment workflow: admin auto-assignment should choose from latest vendor capacity updates and active admin-onboarded drivers; vendors can accept or decline assignments, and declines return the order to Needs Attention for admin reassignment.
- Driver roster rule: drivers are onboarded only through admin (`driver-onboarding`) before dispatch automation can attach them to orders.
